...
This project targets both iPhone and iPad (`TARGETED_DEVICE_FAMILY = 1,2`). The UI uses the horizontal size class to switch to a wider four-column dashboard on iPad while keeping the compact two-column layout on iPhone.

### Build an IPA on macOS
1. Open `DuoLikeAnimation.xcodeproj` in Xcode.
2. Select the `DuoLikeAnimation` scheme and an iPad simulator/device.
3. For a signed IPA, select your Apple Developer team under **Signing & Capabilities**.
4. Choose **Product → Archive**, then **Distribute App** to export the IPA.

The project requires iPadOS/iOS 26.5 or newer as currently configured.
